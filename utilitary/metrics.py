...
def compare_acc(orig, predicted):
    """
    Compares the accuracy of the predicted labels
    """
    suma = 0
    total_good = 0
    for i in range(len(orig)):
        if (orig[i] == 0 or predicted[i] == 0):
            suma +=  orig[i] != predicted[i]
        else:
            dif = abs(orig[i] - predicted[i])
            if dif > 0:
                dif = 1    
            suma +=  dif
        if orig[i] == predicted[i]:
            total_good += 1
    acc = 1 - suma/len(predicted)
    return acc, total_good
